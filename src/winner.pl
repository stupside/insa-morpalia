...
win(X, Y, C) :- 
    neighborhood(X, Y, 1, 0, C, C1), % right
    neighborhood(X, Y, -1, 0, C, C2), % left
    Count is C1 + C2, 
    Count >= 3, !.
    
win(X, Y, C) :- 
    neighborhood(X, Y, 0, 1, C, C1), % up
    neighborhood(X, Y, 0, -1, C, C2), % down
    Count is C1 + C2, 
    Count >= 3, !.
    
win(X, Y, C) :- 
    neighborhood(X, Y, 1, -1, C, C1), % diagonal top-right to bottom-left
    neighborhood(X, Y, -1, 1, C, C2), % diagonal bottom-left to top-right
    Count is C1 + C2, 
    Count >= 3, !.
    
win(X, Y, C) :- 
    neighborhood(X, Y, 1, 1, C, C1), % diagonal top-left to bottom-right
    neighborhood(X, Y, -1, -1, C, C2), % diagonal bottom-right to top-left
    Count is C1 + C2, 
    Count >= 3, !.
